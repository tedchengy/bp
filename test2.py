...
s1=[0 for col in range(len(y1))]
s2=[0 for col in range(len(y1))]


for i in range(len(y1)):
    s1[i]=1/(1+math.exp(-y1[i]))
    s2[i]=1/(1+math.exp(-y2[i]))
...
v = np.random.random(size=(3,2))-0.5
u = np.random.random(size=(2,3))-0.5

sigma=[0 for col in range(n)]
       
for num in range(n):
    
    x=[x1[num],x2[num],x3[num]]
    y=[s1[num],s2[num]]
    x=np.mat(x)
    y=np.mat(y)            
    net2 = w * x.T
    
    hidden1=[0 for row in range(len(net2))]
    for i in range(2):
        hidden1[i]=1/(1+math.exp(-net2[i]))
        
    hidden1=np.mat(hidden1)
    net3 = v * hidden1.T
    
    hidden2=[0 for row in range(len(net3))]
    for i in range(3):
        hidden2[i]=1/(1+math.exp(-net3[i]))
        
    hidden2=np.mat(hidden2)
    net4 = u * hidden2.T
    
    o=[0 for row in range(len(net4))]
    for i in range(2):
        o[i]=1/(1+math.exp(-net4[i]))
    ...
    #反向传播
    delta3=[0 for row in range(len(y.T))]
    for i in range(2):
        delta3[i] = ((y.T[i]-o.T[i])*o.T[i]*(1-o.T[i])).A[0][0]  #转换为数值
    
    delta3=np.mat(delta3)
    
    delta2=[0 for row in range(len(hidden2.T))]
    u=np.mat(u)
    v=np.mat(v)
    w=np.mat(w)
    for j in range(3):
        delta2[j] = (hidden2.T[j]*(1-hidden2.T[j])*delta3*u[:,j]).A[0][0]
    
    delta2=np.mat(delta2)
    
    
    delta1=[0 for row in range(len(hidden1.T))]    
    for k in range(2):
        delta1[k] = (hidden1.T[k]*(1-hidden1.T[k])*delta2*v[:,k]).A[0][0]
    
    delta1=np.mat(delta1)
    
    
    #更新权值
    for i in range(2):
        for j in range(3):
            u[i,j] = u[i,j] + eta*delta3.T[i]*hidden2.T[j];
    
    for i in range(3):
        for j in range(2):
            v[i,j] = v[i,j] + eta*delta2.T[i]*hidden1.T[j];
    
    for i in range(2):
        for j in range(3):
            w[i,j] = w[i,j] + eta*delta1.T[i]*x.T[j];
    
    e=o-y
    sigma[num]=(e*e.T).A[0][0]
